...
class MonsterTest : AnnotationSpec() {
  ...
  private val monstertype = PrimitiveType.WATER
  ...
  fun `check serializable`() {
    ...
    assertThat(monster.primitiveType).isEqualTo(decodedMonster.primitiveType)
  ...
  fun `Monster with Type Wasser has Type Wasser`() {
    assertThat(monster.primitiveType).isEqualTo(monstertype)
  }
  ...
  fun `monster A faster than monster B`() {
    val monsterA =
      Monster("A", BattleStats(100, 100), PrimitiveType.PLANT, listOf())
    val monsterB =
      Monster("B", BattleStats(100, 90), PrimitiveType.WATER, listOf())
  ...
  fun `monster A is not faster than monster B when same speed`() {
    val monsterA =
      Monster("A", BattleStats(100, 100), PrimitiveType.PLANT, listOf())
    val monsterB =
      Monster("B", BattleStats(100, 100), PrimitiveType.WATER, listOf())
